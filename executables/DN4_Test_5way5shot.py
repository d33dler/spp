...
def validate(val_loader, model, criterion, epoch_index, F_txt):
    ...
    for episode_index, (query_images, query_targets, support_images, support_targets) in enumerate(val_loader):
        ...
        model()
        loss = model.module_topology['BACKBONE_2D'].calculate_loss(model.data.sim_list_BACKBONE2D, target)
...
if opt.resume:
    model.load_model(opt.resume, F_txt)
...
for r in range(repeat_num):
    ...
    # =========================================== Evaluation ==========================================
    prec1, accuracies = validate(test_loader, model, model.BACKBONE_2D.criterion, epoch_index, F_txt)
